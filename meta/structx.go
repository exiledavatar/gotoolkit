...
func ToStructx(value any) (Structx, error) {

	s, isStruct := value.(Structx)
	if isStruct {
		return s, nil
	}
...
func (s *Structx) SetUUID(id string) {
	s.UUID = id
	// if s.Fields != nil {
	// 	s.Fields.SetUUID(id)
	// }
...
func NewStructx(value any, cfg StructConfigx) (Struct, error) {
...
func (s Structx) Identifier() string {
...
func (s Structx) ValueMap(tagKey string) ValueMap {
...
func (s Structx) ExecuteTemplate(tpl string, funcs template.FuncMap, data map[string]any) (string, error) {
	d := map[string]any{
